...
def get_vpc_details():
    ...
    for region in regions:
        ...
        for vpc in vpcs:
            ...
            state = vpc['State']
            print(
                f"VPC ID: {vpc_id}, CIDR Block: {cidr_block}, State: {state}")

            # Check if the VPC has associated resources
            associated_resources = check_associated_resources(
                ec2_client_region, vpc_id)
...
# def check_associated_resources(ec2_client, vpc_id):
    # Same check_associated_resources function as before
    # ...
def check_associated_resources(ec2_client, vpc_id):
